...
| --- | --- |
| `WASD` / 方向键 | 走位。**上下是纵深** |
| `J` / 空格 | 攻击，收招段再按接第二段。**按方向键起手可转身打另一侧** |
| `K` / `Shift` | 冲刺，第 2-15 帧无敌。**按着方向键冲就朝那个方向**，包括纵深；第 15 帧起可取消接攻击 |
| `L` / `Q` | 跳跃，弧线位移。**按着方向键跳就朝那个方向**；第 3-27 帧躲开贴地判定（不是全无敌，全场范围技躲不掉）；第 16 帧起可取消接跳劈，跳劈落地后还能再接一次连段。腾空时不能触发冲刺 |
